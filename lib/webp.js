...
async function edit(inputPath, opts, onLog) {
  ...
  try {
    ...
    // fps 결정 (프레임 솎기 전에 확정해야 비율 계산 가능)
    const fps = opts.fps ? clampNum(opts.fps, 0.5, 60, src.fps) : src.fps;

    // 프레임 솎기 — fps를 낮출 경우 재생 속도를 유지하면서 프레임 수를 줄인다
    // (프레임 수를 그대로 두고 fps만 낮추면 애니메이션이 느려지는 문제 방지)
    const keepIdx = [];
    if (fps < src.fps * 0.99) {
      // 균등 샘플링: 전체 구간에서 targetCount개를 고르게 뽑음
      const targetCount = Math.max(1, Math.round(src.pages * fps / src.fps));
      for (let i = 0; i < targetCount; i++) {
        keepIdx.push(Math.min(src.pages - 1, Math.round(i * src.pages / targetCount)));
      }
    } else {
      for (let i = 0; i < src.pages; i++) keepIdx.push(i);
    }
    ...
    if (keepIdx.length !== src.pages) log(`▶ 프레임 솎기: ${src.pages} → ${keepIdx.length} (속도 유지)\n`);
