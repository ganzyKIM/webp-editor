...
/** 영상 직접 업로드용 서명 URL 생성 (Supabase only). 로컬 시 null 반환. */
async function createSignedUploadUrl(token) {
  if (!USE_SUPABASE) return null;
  const { data, error } = await sb.storage.from(BUCKET).createSignedUploadUrl(token);
  if (error) throw new Error('서명 URL 생성 실패: ' + error.message);
  return { signedUrl: data.signedUrl, uploadToken: data.token };
}

module.exports = { put, get, remove, startLocalCleanup, createSignedUploadUrl, USE_SUPABASE };
